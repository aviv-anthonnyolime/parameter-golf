...
class GPT(nn.Module):
    def __init__(
        ...
        self.depth_embed = nn.Embedding(
            self.num_encoder_layers + self.num_decoder_layers, model_dim
        )
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, lora=None) -> Tensor:
        ...
        for i in range(self.num_decoder_layers):
            ...
            x = x + self.depth_embed.weight[bi].to(dtype=x.dtype)[None, None, :]
            if skips:
                x = (
                    x
