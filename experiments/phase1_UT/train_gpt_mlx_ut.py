...
class Hyperparameters:
    ...
    num_unique_layers: int = int(os.environ.get("NUM_UNIQUE_LAYERS", 1))
...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, num_layers: int, num_unique_layers: int, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
        ...
        self.num_steps_recurrence = (self.num_encoder_layers + self.num_decoder_layers) // self.num_unique_layers
        self.num_skip_weights = min(self.num_encoder_layers, self.num_decoder_layers)
        self.skip_weights = mx.ones((self.num_skip_weights, dim), dtype=mx.float32)
        self.depth_embed = nn.Embedding(self.num_encoder_layers + self.num_decoder_layers, dim)
        self.blocks = [
    ...
    def __call__(self, input_ids: mx.array) -> mx.array:
        ...
        for i in range(self.num_encoder_layers):
            x = x + self.depth_embed.weight[i].astype(x.dtype)[None, None, :]
        ...
        for i in range(self.num_decoder_layers):
            ...
            x = x + self.depth_embed.weight[bi].astype(x.dtype)[None, None, :]
            if skips:
                x = x + self.skip_weights[i].astype(x.dtype)[None, None, :] * skips.pop()
            x = self.blocks[bi % self.num_unique_layers](x, x0)
...
class SplitOptimizers:
    ...
    def __init__(self, model: GPT, args: Hyperparameters):
        ...
        self.scalar_keys = [
            k
            for k, p in params.items()
            if k in ("skip_weights", "depth_embed.weight") or (k.startswith("blocks.") and (p.ndim < 2 or any(pattern in k for pattern in CONTROL_TENSOR_NAME_PATTERNS)))
        ]
