...
def main() -> None:
    ...
    # --- Run naming & log file (early, needed by log0) ---
    now = datetime.datetime.now()
    date_str = now.strftime("%Y-%m-%d")
    time_str = now.strftime("%H%M%S")
    run_name = "baseline"
    ptag = f"{args.num_layers}L-{args.model_dim}d-{args.mlp_mult}mlp"
    experiment = Path(__file__).resolve().parent.name

    logfile = None
    if master_process:
        os.makedirs("logs", exist_ok=True)
        log_fname = f"{date_str}_{time_str}_{ptag}_{run_name}.txt"
    ...
    wandb_run_name = f"{run_name}_{ptag}"
    if master_process:
    ...
    log0(f"train_time:{training_time_ms:.0f}ms")
    ...
    if wandb_run is not None:
        wandb_run.summary.update({
            ...
            "val_bpb_int8": float(q_val_bpb),
            "val_bpb_ttt": float(ttt_val_bpb) if ttt_val_bpb is not None else None,
            "compressed_size_mb": round(quant_file_bytes / 1024**2, 2),
            "total_steps": step,
            "train_time_s": round(training_time_ms / 1000.0, 1),
        })
        wandb_run.finish()
